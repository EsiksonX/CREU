...
class CREU(nn.Module):
    ...
    def build(self,
        ...
        self.log_v_encoder = nn.ModuleList([
            nn.Sequential(
                nn.Linear(emb_dim, 32),
                nn.LeakyReLU(),
                nn.Linear(32, 16)
        ...
        self.mean_encoder_p = nn.ModuleList([
            nn.Sequential(
                nn.Linear(self.output_emb_size, 32),
                nn.LeakyReLU(),
                nn.Linear(32, 16)
            )
            for _ in range(self.num_components)
        ])
        self.log_v_encoder_p = nn.ModuleList([
            nn.Sequential(
                nn.Linear(self.output_emb_size, 32),
                nn.LeakyReLU(),
                nn.Linear(32, 16)
        ...
        self.pi = nn.Parameter(torch.ones(self.num_components) / self.num_components)
        # self.pi_layer = nn.Linear(self.output_emb_size, self.n_components)
    ...
    def epistemic_uncertainty(self, outputs):
        M = self.num_components
        distance = SamplesLoss("sinkhorn", p=2, blur=0.05)

        D = torch.zeros((M, M), device=self.device)
        for i in range(M):
            for j in range(M):
                D[i, j] = distance(outputs[i][0], outputs[j][0])
        exp_neg_D = torch.exp(-D)
        inner_sum = torch.matmul(self.pi, exp_neg_D)
        uncertainty = -torch.sum(self.pi * torch.log(inner_sum))
    ...
    def warm_item_id(self, x_dict):
        ...
        for i in range(self.num_components):
            mean = self.mean_encoder[i](item_id_emb)
            log_v = self.log_v_encoder[i](item_id_emb)
            mean_p = self.mean_encoder_p[i](sideinfo_emb)
            log_v_p = self.log_v_encoder_p[i](sideinfo_emb)
            ...
            freq = x_dict['count']
            pred = self.decoders[i](torch.concat([z, freq], 1))
            pred_p = self.decoders[i](torch.concat([z_p, freq], 1))
            recon_term = torch.square(pred - item_id_emb).sum(-1).mean()

            outputs.append((pred_p, reg_term, recon_term))

        return outputs

    def forward(self, x_dict):
        outputs = self.warm_item_id(x_dict)
        warm_id_emb = outputs[0][0]
        reg_term = outputs[0][1]
        recon_term = outputs[0][2]
        target = self.model.forward_with_item_id_emb(warm_id_emb, x_dict)

        uncertainty = self.epistemic_uncertainty(outputs)

        return target, uncertainty, recon_term, reg_term
